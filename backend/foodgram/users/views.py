...
class FollowUserViewSet(UserViewSet):
    ...
    def subscribe(self, serializer, id=None):
        ...
        if self.request.method == 'GET':
            ...
            try:
                Follow.objects.get(user=self.request.user, following=following_user)
                return Response(status=status.HTTP_400_BAD_REQUEST)

            except Follow.DoesNotExist:
                follow = Follow.objects.create(user=self.request.user, following=following_user)
                return Response(FollowUsersSerializer(follow).data)
        
        if self.request.method == 'DELETE':
            try:
                Follow.objects.get(user=self.request.user, following=following_user).delete()
